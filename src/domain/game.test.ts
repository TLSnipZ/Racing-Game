...
describe('KAGEHAMA core game', () => {
  it('creates the expected new game state', () => {
    expect(createNewGameState()).toEqual({ cashYen: STARTING_CASH_YEN, playerLevel: 1, reputation: 0,
      selectedStarterId: null, ownedVehicles: [], activeVehicleId: null });
  });
  it.each([['pico-rs', 18000], ['tora-85', 8000], ['rz-t', 2000]] as const)('buys and activates %s for the real price', (id, cash) => {
    ...
    const state = purchaseStarter(initial, id, 'vehicle-001');
    expect(state.cashYen).toBe(cash);
    expect(state.selectedStarterId).toBe(id);
    expect(state.ownedVehicles).toHaveLength(1);
    expect(state.activeVehicleId).toBe('vehicle-001');
    expect(initial).toEqual(createNewGameState());
  });
  ...
  it('creates an individual vehicle with condition, mileage, originality and stock parts', () => {
    ...
    expect(vehicle).toMatchObject({ instanceId: 'akari-instance-9', catalogId: 'rz-t', engineCondition: 57,
      bodyCondition: 61, transmissionCondition: 56, originality: 88, odometerKm: 189340 });
    expect(vehicle.installedParts).toContain('Factory turbo');
  });
  it('keeps independent stock-parts arrays', () => {
    const a = createPlayerVehicle('pico-rs', 'a');
    const b = createPlayerVehicle('pico-rs', 'b');
    a.installedParts.push('Test part');
    expect(b.installedParts).not.toContain('Test part');
    expect(STARTER_CARS[0].stockParts).not.toContain('Test part');
  });
  it.each([0, 31000, NaN, Infinity, -1, 50000.5])('rejects invalid/insufficient cash %s', (cashYen) => {
    expect(() => purchaseStarter({ ...createNewGameState(), cashYen }, 'pico-rs', 'a')).toThrow();
  });
