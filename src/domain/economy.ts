...
/** Structural safety, not anti-cheat. Save codes and the client clock are player-controlled. */
export function isEconomyState(value: unknown, vehicles: readonly PlayerVehicle[]): value is EconomyState {
  ...
  if (last === null) {
    ...
    if (!record(last) || !integer(last.runId, 1) || last.runId >= value.nextRunId
      || typeof last.jobId !== 'string' || !findJob(last.jobId)
      || !reward(last.rewardYen) || !reward(last.reputationReward)
  ...
  if (!record(active) || !integer(active.runId, 1) || active.runId !== value.nextRunId - 1
    || typeof active.jobId !== 'string' || !integer(active.startedAtMs)
    || !integer(active.finishesAtMs) || active.finishesAtMs <= active.startedAtMs
    || active.finishesAtMs - active.startedAtMs > 86_400_000
  ...
  return definition.requiresVehicle
    ? typeof active.vehicleId === 'string' && vehicles.some((vehicle) => vehicle.instanceId === active.vehicleId)
...
function assertClock(nowMs: number) {
  if (!integer(nowMs)) throw new Error('Device clock is invalid. Restore your clock and retry.');
}
...
/** Pure command. The session persists this state before showing the accepted contract. */
export function startJob(state: GameState, jobId: string, nowMs: number): GameState {
  ...
  return { ...state, economy: { ...state.economy, nextRunId, activeJob: {
    ...
    startedAtMs: nowMs, finishesAtMs, rewardYen: job.rewardYen,
    reputationReward: job.reputationReward, distanceKm: job.distanceKm,
  } } };
}

/** Claim must name the current run. Stale/double clicks cannot pay a different contract. */
export function claimJob(state: GameState, runId: number, nowMs: number): GameState {
  ...
  // Mileage belongs to the accepted vehicle, even if another car was activated meanwhile.
