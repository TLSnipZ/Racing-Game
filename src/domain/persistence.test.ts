import { withoutCollection } from './collectionTestUtils';
...
describe('Current save and historical compatibility', () => {
  ...
  it('migrates the actual Phase 2 shape without losing any original field', () => {
    ...
    expect({ ...result, state: withoutCollection(result.state) }).toEqual({ ...legacy, version: SAVE_VERSION, state: { ...legacy.state, activeVehicleId: 'legacy-pico-001', economy: createEconomyState(), racing: createRacingState(), heat: createHeatState(), market: createMarketState(),
  ...
  it('migrates a fixed v2 fixture without a reward, reset or changed active car', () => {
    ...
    expect({ ...result, state: withoutCollection(result.state) }).toEqual({ ...legacyV2, version: SAVE_VERSION, state: { ...legacyV2.state, economy: createEconomyState(), racing: createRacingState(), heat: createHeatState(), market: createMarketState(),
  ...
  it('preserves a second active car and valid legacy levels in v2', () => {
    ...
    const { economy: _economy, racing: _racing, ...old } = state; expect(withoutCollection(deserializeSave(raw(old, 2)).state)).toEqual(withoutCollection(state));
  });
...
describe('Tuning migration and invalid imports', () => {
  it('preserves a fixed v3 pending delivery, previous receipt, car and balance exactly', () => {
    ...
    expect(withoutCollection(result.state)).toEqual({ ...legacyV3.state, racing: createRacingState(), heat: createHeatState(), market: createMarketState(), ownedVehicles: legacyV3.state.ownedVehicles.map((v) => ({ ...v, tuning: createVehicleTuning() })) });
