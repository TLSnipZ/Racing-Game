...
describe('Current save and historical compatibility', () => {
  ...
  it('migrates the actual Phase 2 shape without losing any original field', () => {
    ...
    expect(result).toEqual({ ...legacy, version: SAVE_VERSION, state: { ...legacy.state, activeVehicleId: 'legacy-pico-001', economy: createEconomyState(), market: createMarketState(), racing: createRacingState(),
  ...
  it('migrates a fixed v2 fixture without a reward, reset or changed active car', () => {
    ...
    expect(result).toEqual({ ...legacyV2, version: SAVE_VERSION, state: { ...legacyV2.state, economy: createEconomyState(), market: createMarketState(), racing: createRacingState(),
...
describe('Tuning migration and invalid imports', () => {
  it('preserves a fixed v3 pending delivery, previous receipt, car and balance exactly', () => {
    ...
    expect(result.state).toEqual({ ...legacyV3.state, market: createMarketState(), racing: createRacingState(), ownedVehicles: legacyV3.state.ownedVehicles.map((v) => ({ ...v, tuning: createVehicleTuning() })) });
