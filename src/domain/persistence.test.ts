...
describe('Save v3 and v1/v2 compatibility', () => {
  ...
  it('preserves a second active car and valid legacy levels in v2', () => {
    const state = purchased(); state.ownedVehicles.push(createPlayerVehicle('rz-t', 'second')); state.activeVehicleId = 'second';
    state.playerLevel = 7; state.reputation = 100;
    const { economy: _economy, ...old } = state;
    expect(deserializeSave(raw(old, 2)).state).toEqual(state);
  });
  it.each([legacy, legacyV2])('imports old schema $version codes without a new prefix', (fixture) => {
    ...
    expect(importSaveCode(code).state.economy).toEqual(createEconomyState());
    expect(importSaveCode(code).state.cashYen).toBe(fixture.state.cashYen);
  });
  it('migrates an empty v1 save to a null active ID and empty economy', () => {
    const { activeVehicleId: _id, economy: _economy, ...old } = createNewGameState();
  ...
  it('rejects a missing v2 active ID instead of silently migrating v2', () => {
    const { activeVehicleId: _id, economy: _economy, ...state } = purchased();
    expect(() => deserializeSave(raw(state, 2))).toThrow();
  });
  it('rejects missing v3 economy instead of treating v3 as a legacy save', () => {
    const { economy: _economy, ...state } = purchased(); expect(() => deserializeSave(raw(state))).toThrow();
  });
  ...
  it('round-trips pending and completed jobs including deadlines and receipts', () => {
    const pending = startJob(purchased(), 'garage-shift', 1000);
    expect(importSaveCode(exportSaveCode(pending, 1234)).state).toEqual(pending);
    const paid = claimJob(pending, 1, 16000);
    expect(deserializeSave(serializeSave(paid, 17000)).state).toEqual(paid);
  });
  it('migration and decoding do not claim a completed wall-clock timer', () => {
    const pending = startJob(purchased(), 'garage-shift', 1000);
    const result = deserializeSave(serializeSave(pending, 2_000_000));
    expect(result.state.cashYen).toBe(18000); expect(result.state.economy.activeJob?.runId).toBe(1);
  });
  it.each([
    { runId: 99 }, { jobId: 'unknown' }, { startedAtMs: -1 }, { finishesAtMs: 1000 },
    { rewardYen: -1 }, { reputationReward: 1.5 }, { distanceKm: -1 }, { vehicleId: 'ghost' },
  ])('rejects malformed active job %j', (patch) => {
    const state = startJob(purchased(), 'garage-shift', 1000);
    const malformed = { ...state, economy: { ...state.economy, activeJob: { ...state.economy.activeJob, ...patch } } };
    expect(() => deserializeSave(raw(malformed))).toThrow();
  });
  it('rejects an unowned delivery vehicle', () => {
    const state = startJob({ ...purchased(), playerLevel: 2, reputation: 20 }, 'parts-run', 1000);
    state.economy.activeJob!.vehicleId = 'ghost'; expect(isGameState(state)).toBe(false);
  });
  ...
  it('rejects malformed receipts and replayed active run IDs', () => {
    const paid = claimJob(startJob(purchased(), 'garage-shift', 1000), 1, 16000);
    const state = startJob(paid, 'garage-shift', 20000);
    state.economy.lastReceipt!.runId = 2; expect(isGameState(state)).toBe(false);
    paid.economy.lastReceipt!.levelAfter = 0; expect(isGameState(paid)).toBe(false);
  });
});
