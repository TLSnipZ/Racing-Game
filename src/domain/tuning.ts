...
export function getInstalledUpgrades(vehicle: PlayerVehicle): PartDefinition[] {
  return TUNING_SLOTS.flatMap((slot) => {
    const id = vehicle.tuning.installedBySlot[slot];
...
export function getVehicleBuildStats(vehicle: PlayerVehicle): VehicleBuildStats {
  ...
  const mods = getInstalledUpgrades(vehicle).reduce((sum, part) => ({
    powerBps: sum.powerBps + (part.modifiers.powerBps ?? 0),
    weightKg: sum.weightKg + (part.modifiers.weightKg ?? 0),
  ...
  // BigInt keeps the integer-PS calculation safe even for deliberately edited high-value saves.
  ...
  const powerPs = Number(rounded);
  const weightKg = Math.max(250, vehicle.weightKg + mods.weightKg);
...
/** Show fitted replacements while preserving the old factory-parts strings in the save. */
export function getFittedPartNames(vehicle: PlayerVehicle): string[] {
  ...
  return [...vehicle.installedParts.filter((name) => !installed.some((part) => words[part.slot].test(name))),
    ...installed.map((part) => part.name)];
}
...
export function isVehicleBusy(state: GameState, instanceId: string): boolean {
  return state.economy.activeJob?.vehicleId === instanceId;
}
export function getPartRequirement(state: GameState, vehicle: PlayerVehicle, part: PartDefinition): string | null {
  ...
  if (isVehicleBusy(state, vehicle.instanceId)) return 'Vehicle assigned to a job. Claim or cancel it before tuning.';
...
/** One atomic buy-and-fit command. Owned parts can be reinstalled, but never charged twice. */
export function installPart(state: GameState, instanceId: string, partId: string, expectedInstalledId: string | null): GameState {
  ...
  getVehicleBuildStats(updated); // Reject impossible results before charging or publishing any state.
...
export function removePart(state: GameState, instanceId: string, slot: TuningSlot, expectedPartId: string): GameState {
  ...
  if (isVehicleBusy(state, instanceId)) throw new Error('Vehicle assigned to a job. Claim or cancel it before tuning.');
  const installedBySlot = { ...vehicle.tuning.installedBySlot };
  delete installedBySlot[slot];
