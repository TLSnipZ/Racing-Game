...
export function SaveManagement({ game, blocked, onImport, onReset }: {
  ...
  // A generated code is a snapshot: clear it after a real game-state change.
  ...
  return <section id="save-data" className="savePanel phase3SavePanel" aria-label="Save management">
    ...
    <div className="saveGrid">
      ...
      <article><div className="saveIcon"><Upload /></div><h4>Import save</h4>
        <p>Codes are checked first. Confirm the preview to replace your current progress.</p>
        <textarea className="importBox" value={importCode} maxLength={MAX_SAVE_LENGTH} onChange={(e) => setImportCode(e.target.value)}
          placeholder="KAGEHAMA1-..." aria-label="Save code to import" />
        <button type="button" className="secondaryButton" disabled={!importCode.trim()} onClick={importCodeNow}>VALIDATE & IMPORT</button>
      </article>
      <article className="dangerCard"><div className="saveIcon"><RotateCcw /></div><h4>Reset savegame</h4>
        <p>Back to ¥50,000 and three starter choices. Export a backup before resetting.</p>
        <button type="button" className="dangerButton" onClick={reset}>RESET SAVEGAME</button>
      </article>
    </div>
    {message && <p className="saveFeedback" role="status">{message}</p>}
    {error && <div className="gameError" role="alert">{error}</div>}
