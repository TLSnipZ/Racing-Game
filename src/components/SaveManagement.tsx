...
export function SaveManagement({ game, blocked, onImport, onReset }: {
  ...
  function importCodeNow() {
    try {
      ...
      if (!window.confirm(`Import this save?\n\nCash: ¥${state.cashYen.toLocaleString('en-US')}\nLevel: ${state.playerLevel}\nCollected models: ${state.collection.collectedModelIds.length}\nClaimed achievement rewards: ${state.collection.claimedAchievementIds.length}\nUsed Icon offers: ${state.collection.purchasedIconIds.length}\nVehicles: ${state.ownedVehicles.length}\nActive: ${getActiveVehicle(state)?.name ?? 'None'}\nMarket batch: ${market.generation + 1} (${market.listings.length} unsold listings)\nMarket trades: ${market.purchasedCount} bought / ${market.soldCount} sold\nPurchased parts: ${state.ownedVehicles.reduce((n, car) => n + car.tuning.purchasedPartIds.length, 0)}\nPending job: ${job ? `${job.jobId} (#${job.runId})` : 'None'}\nPending race: ${race ? `${race.eventName} (#${race.runId}), entry already paid` : 'None'}\nRace finishes: ${state.racing.completedRaces}${heatSummary}\nSpecialist contract: ${state.advanced.activeContract?.offerId ?? 'None'}\nBusinesses: ${state.empire.businesses.length} (${state.empire.businesses.filter((b) => b.managerHired).length} managers)\nGarage expansion tier: ${state.empire.garageExpansionLevel}\nBusiness profit collected: ¥${state.empire.totalCollectedYen.toLocaleString('en-US')}\nBusiness production anchors and tills: retained, not reset\nSpecialist sources acquired: ${state.advanced.acquiredOfferIds.length}\n\nYour current save, including collection history, achievement claims, Icon offers, market stock/history, parts, race records and any pending job or race, will be replaced.`)) return;
  ...
  function reset() {
    if (!window.confirm('Reset your entire KAGEHAMA save?\n\nBusinesses, managers, uncollected production and garage expansions will be reset.\nHeat, patrol alerts and Lay low progress will also be cleared.\nCollection history, achievement claims, Icon offers, cars, market stock/history, parts, money, progress, race records and any pending job or race will be reset. This cannot be undone unless you exported a save code first.')) return;
  ...
  return <section id="save-data" className="savePanel phase3SavePanel" aria-label="Save management">
    ...
    <div className="saveGrid">
      <article><div className="saveIcon"><Download /></div><h4>Export save</h4><p>Back up businesses, managers, production anchors, garage expansions, specialist escrow, collection history, claimed rewards, Icon offers, Heat, patrol alerts, cooldowns, cars, market stock, tuning, progress, race records and your pending activity, or move your save to another device.</p>
