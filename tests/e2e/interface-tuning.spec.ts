...
test('real tabs expose exactly one panel and never mutate the session', async ({ page }) => {
  ...
  for (const name of ['Jobs', 'Workshop', 'Saves', 'Garage'] as const) {
  ...
  await expect(page.getByRole('tab', { name: 'City', exact: true })).toBeDisabled(); await expect(page.getByRole('tab', { name: 'Races', exact: true })).toBeDisabled();
});
