...
async function seed(page: Page, raw: string) {
  await page.addInitScript(({ key, data }) => {
    if (localStorage.getItem(key) === null) localStorage.setItem(key, data);
  }, { key: SAVE_STORAGE_KEY, data: raw });
  await page.goto('./');
}
const stored = (page: Page) => page.evaluate((key) => localStorage.getItem(key), SAVE_STORAGE_KEY);

test('three starters, real purchase, active vehicle and reload', async ({ page }) => {
  await page.goto('./');
  await expect(page.getByRole('heading', { name: 'Choose your starter' })).toBeVisible();
  await expect(page.getByRole('button', { name: /^Choose (Hoshino|Akari)/ })).toHaveCount(3);
  await page.getByRole('button', { name: 'Choose Hoshino Pico RS' }).click();
  await page.getByRole('button', { name: 'BUY & ENTER KAGEHAMA' }).click();
  await expect(page.getByTestId('cash')).toHaveText('¥18,000');
  await expect(page.locator('.activeSummary strong')).toHaveText('Hoshino Pico RS');
  const before = JSON.parse((await stored(page))!);
  await page.reload();
  await expect(page.locator('.activeSummary strong')).toHaveText('Hoshino Pico RS');
  const after = JSON.parse((await stored(page))!);
  expect(after.state).toEqual(before.state); expect(after.version).toBe(SAVE_VERSION);
});
...
test('migrates a v1 browser save with its original instance and balance', async ({ page }) => {
  ...
  expect(JSON.parse((await stored(page))!).state).toEqual({ ...legacy.state, activeVehicleId: 'legacy-pico-001', economy: createEconomyState() });
});

test('inspection is free and does not change active ride; explicit activation persists', async ({ page }) => {
  await seed(page, serializeSave(twoCars()));
  await page.getByRole('button', { name: 'Inspect Akari RZ-T (akari-1)' }).click();
  await expect(page.locator('.showcaseCopy h3')).toHaveText('Akari RZ-T');
  await expect(page.locator('.activeSummary strong')).toHaveText('Hoshino Pico RS');
...
test('export, confirmed reset and import restore the entire multi-car garage', async ({ page }) => {
  const game = { ...twoCars(), activeVehicleId: 'akari-1' };
  await seed(page, serializeSave(game));
  ...
  await page.getByRole('button', { name: 'RESET SAVEGAME' }).click();
  await expect(page.getByRole('heading', { name: 'Choose your starter' })).toBeVisible();
  await expect(page.getByTestId('cash')).toHaveText('¥50,000');
  await page.reload();
  await expect(page.getByRole('heading', { name: 'Choose your starter' })).toBeVisible();
...
test('imports a Phase 2 KAGEHAMA1 code after confirmation', async ({ page }) => {
  await page.goto('./');
  const code = 'KAGEHAMA1-' + Buffer.from(JSON.stringify(legacy)).toString('base64url');
  await page.getByRole('textbox', { name: 'Save code to import' }).fill(code);
  page.once('dialog', (dialog) => dialog.dismiss());
  await page.getByRole('button', { name: 'VALIDATE & IMPORT' }).click();
  await expect(page.getByRole('heading', { name: 'Choose your starter' })).toBeVisible();
  ...
  await page.getByRole('button', { name: 'VALIDATE & IMPORT' }).click();
  await expect(page.locator('.activeSummary strong')).toHaveText('Hoshino Pico RS');
  expect(JSON.parse((await stored(page))!).state.activeVehicleId).toBe('legacy-pico-001');
});

test('invalid imports leave the current save untouched', async ({ page }) => {
  await seed(page, serializeSave(twoCars()));
  await expect(page.locator('.activeSummary strong')).toBeVisible();
  const before = await stored(page);
  await page.getByRole('textbox', { name: 'Save code to import' }).fill('not-a-save');
  await page.getByRole('button', { name: 'VALIDATE & IMPORT' }).click();
  await expect(page.getByRole('alert')).toContainText('not a KAGEHAMA');
...
for (const [name, data] of [['corrupt', '{broken'], ['future', JSON.stringify({ ...legacy, version: 99 })]]) {
  test(`${name} stored data is protected, not deleted or overwritten`, async ({ page }) => {
    ...
    await expect(page.getByRole('alert')).toContainText('stored save is protected');
    expect(await stored(page)).toBe(data);
...
test('search, sorting and mobile layout do not mutate or overflow the garage', async ({ page }) => {
  ...
  await expect(page.locator('.ownedCard')).toHaveCount(1);
  await page.getByRole('searchbox', { name: 'Search owned vehicles' }).fill('');
  await page.getByRole('combobox', { name: 'Sort owned vehicles' }).selectOption('power');
  await expect(page.locator('.ownedCard').first()).toContainText('Akari RZ-T');
  expect(await stored(page)).toBe(before);
...
test('desktop visual smoke test has no page errors', async ({ page }) => {
  ...
  await expect(page.getByRole('meter', { name: 'Engine', exact: true })).toHaveAttribute('aria-valuenow', '88');
...
test('a second tab cannot silently overwrite a newly selected active car', async ({ page, context }) => {
  ...
  await other.getByRole('button', { name: 'Inspect Akari RZ-T (akari-1)' }).click();
  await other.getByRole('button', { name: 'SET AS ACTIVE VEHICLE' }).click();
