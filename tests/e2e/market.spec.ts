...
test('a frozen v5 save migrates without losing tuned parts, money or its unsettled paid race', async ({ page }) => {
  ...
  const { collection, heat, market, ...old } = await read(page); expect(old).toEqual(legacyV5.state); expect(heat).toEqual(createHeatState()); expect(market.listings).toHaveLength(6); expect(market.purchasedCount).toBe(0);
